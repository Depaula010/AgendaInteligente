...
* [x] **B18–B23 — Bot WhatsApp: Loop completo, Onboarding e features de suporte:**
  * [x] **B18 — Fechar loop bot↔backend:** `IBotIntentDispatcherService` + `BotIntentDispatcherService` — despacha intenções `schedule`/`cancel` para domínio (find-or-create customer, `ScheduleService.CreateAsync`, `UpdateStatusAsync`). `WebhookService` atualizado: debounce → histórico → IA → dispatcher → salvar histórico → enviar resposta. `ICustomerRepository` + `CustomerRepository` + `IScheduleRepository.GetUpcomingByCustomerIdAsync`.
  * [x] **B19 — Endpoint de resposta ao WhatsApp:** `POST /api/v1/whatsapp/send` (JWT + `RequireOwnerRole`). Canal único de saída para o bot Node.js. Retorna 502 quando bot indisponível. `SendWhatsAppRequest`, `SendWhatsAppResponse`, `WhatsAppEndpoints`.
  * [x] **B20 — Estado de conversa Redis:** `ConversationHistoryService` migrado de `AbsoluteExpiration` para `SlidingExpiration` (24h) — preserva contexto em conversas async WhatsApp onde o usuário responde horas depois.
  * [x] **B21 — Template de conflito configurável:** `TenantSettings.ConflictMessageTemplate` (nullable, placeholder `{alternatives}`). Migration `AddConflictMessageTemplateToTenantSettings`. `BotIntentDispatcherService` lê o template do banco via `ITenantSettingsRepository`; fallback para `DefaultConflictTemplate` hardcoded. `InternalsVisibleTo` adicionado em `Properties/AssemblyInfo.cs` para acesso ao `internal const` nos testes.
  * [x] **B22 — Registro automático de Customer:** `WebhookService` executa find-or-create antes da IA para qualquer mensagem de número desconhecido. Early return em mensagens duplicadas preservado — nenhum customer espúrio criado. `TenantId` definido explicitamente (Customer não implementa `IMustHaveTenant`).
